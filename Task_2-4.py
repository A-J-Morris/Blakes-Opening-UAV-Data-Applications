...
@detail:         Task 2.4: This program will implement kernel operations on
...
output_file_name_a = data_location / "mean_slope_raster.tif"
output_file_name_b = data_location / "min_slope_raster.tif"
output_file_name_c = data_location / "max_slope_raster.tif"
...
KERNEL_SIZE = 3 # Determines the dimensions of the kernel i.e 5 = 5x5
...
def raster_fledermaus_mean_min_max(inarr, nrows, ncols):
    ...
    for r in range(KERNEL_OFFSET, nrows - KERNEL_OFFSET):
        for c in range(KERNEL_OFFSET, ncols - KERNEL_OFFSET):
            ...
            #The indexing for west_vals, etc.. only work for the 3*3 array - needs to be updated for task 2.5
            
            # Slope for West Arm
            west_slope = np.arctan((west_vals[0][0] - c_val) / (2 * CELL_SIZE))
            
            # Slope for East Arm
            east_slope = np.arctan((east_vals[0][0] - c_val) / (2 * CELL_SIZE))
            
            # Slope for North Arm
            north_slope = np.arctan((north_vals[0][0] - c_val) / (2 * CELL_SIZE))

            # Slope of the South Arm
            south_slope = np.arctan((south_vals[0][0] - c_val) / (2 * CELL_SIZE))            
            
            # The average slope as I interpreted from the spec (*The reported resulted is the arctan function of the average value of the four slopes, coverted to degrees)
...
def plot_mean_dem(slope_raster):
    ...
    plt.title('Homehill Landslide: Slope Raster (Using Mean Kernel)')
    plt.show()
...
def plot_min_dem(slope_raster):
    ...
    plt.ylabel('Pixel No. (y)', labelpad = 20)
    plt.xlabel('Pixel No. (x)', labelpad = 20)
    plt.title('Homehill Landslide: Slope Raster (Using Min Kernel Values)')
    plt.show()
...
def plot_max_dem(slope_raster):
    ...
    plt.ylabel('Pixel No. (y)', labelpad = 20)
    plt.xlabel('Pixel No. (x)', labelpad = 20)
    plt.title('Homehill Landslide: Slope Raster (Using Max Kernel Values)')
    plt.show()
